...
RBNode<T>* RBNode<T>::fix_up(std::unique_ptr<RBNode<T>>& n) {
    if(n->left==nullptr&&n->right&&!is_red(n->right))
        n->flip_color();
    if(n->right==nullptr&&n->left&&!is_red(n->left))
        n->flip_color();
    ...
    if(n->left&&n->left->right&&is_red(n->left)&&is_red(n->left->right)){
        n->left = std::move((std::unique_ptr<RBNode<T>>)rotate_left(n->left));
        n = std::move((std::unique_ptr<RBNode<T>>)rotate_right(n));
    }
    
    if(n->right&&n->right->right&&is_red(n->right->right)){
        if(!n->right->left||!is_red(n->right->left)){
            n->right = std::move((std::unique_ptr<RBNode<T>>)rotate_left(n->right));
        }
    }
...
RBNode<T>* RBNode<T>::remove_max(std::unique_ptr<RBNode<T>>& n) {
    if(n->left&&is_red(n->left))
        if(n->right==nullptr||!is_red(n->right))
            n = std::move((std::unique_ptr<RBNode<T>>)rotate_right(n));
    if(n->right == nullptr)
        return nullptr;
...
RBNode<T>* RBNode<T>::move_red_left(std::unique_ptr<RBNode<T>>& n) {
    n->flip_color();
    if(n->right->left&&is_red(n->right->left)){
        n->right = std::move((std::unique_ptr<RBNode<T>>)rotate_right(n->right));
        n = std::move((std::unique_ptr<RBNode<T>>)rotate_left(n));
        n->flip_color();
    }
    return (n.release());
...
RBNode<T>* RBNode<T>::remove_min(std::unique_ptr<RBNode<T>>& n) {
    if(n->left == nullptr)
        return nullptr;
    if(n->left&&n->left->left&&!is_red(n->left)&&!is_red(n->left->left))
        n = std::move((std::unique_ptr<RBNode<T>>)move_red_left(n));
    n->left = std::move((std::unique_ptr<RBNode<T>>)remove_min(n->left));
    return fix_up(n);
    // TODO
    return nullptr;
}
...
RBNode<T>* RBNode<T>::remove(std::unique_ptr<RBNode<T>>& n, const T& t) {
    if (t < n->key) {
          if (n->left&&n->left->left && !is_red(n->left) && !is_red(n->left->left))
              n = std::move((std::unique_ptr<RBNode<T>>)move_red_left(n));
          n->left = std::move((std::unique_ptr<RBNode<T>>)remove(n->left, t));
       }
       else {
          if (n->left && is_red(n->left))
              if(n->right==nullptr||!is_red(n->right))
          ...
          if (t == n->key) {
             ...
             n->right = std::move((std::unique_ptr<RBNode<T>>)remove(n->right, n->key));
          }
          else n->right = std::move((std::unique_ptr<RBNode<T>>)remove(n->right, t));
       }
       return fix_up(n);
    // TODO
    return nullptr;
...
//
        ...
        const auto& all_leaves = tree.collect_all_leaves();
        for (const auto& [p, n] : all_leaves)
                 std::cout << "path: " << std::bitset<16>{p.p_} << "\tlen: " << p.len_<< "\tblack: " << p.num_black_ << '\n';
        return 0;
}
